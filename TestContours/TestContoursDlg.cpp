...
void CTestContoursDlg::OnBnClickedButton5()
{
	...
	ReadObject(&m_ho_RectanglesPadsIsol, "C:\\Temp1\\RectanglesPadsIsol.hobj");
	CountObj(m_ho_RectanglesPadsIsol, &tnum);
	cstr.Format(_T("(%d)"), (int)tnum);
	SetDlgItemText(IDC_EDIT8, cstr);
	printf("\n***ContoursGrsmPi.hobj and RectanglesPadsIsol.hobj read***");
...
void CTestContoursDlg::OnBnClickedButton11() //Isolated pad single cycle
{
	...
	HObject ho_PadDefect, ho_Rect;
	HTuple hv_DOut;
	HTuple lht1, lht3, lht2, lht, hv_meander;

	SelectObj(m_ho_RectanglesPadsIsol, &m_ho_Rectangle5, m_hv_pad);
	SelectObj(m_ho_ContoursGrsmPi, &m_ho_CG, m_hv_pad);
	CountSeconds(&lht1);
	for (int i = 0; i < num; i++)
...
{
	HObject ho_PadDefectC, ho_Rect;
	HTuple hv_DOut;
	HTuple lht1, lht3, lht2, lht;

	SelectObj(m_ho_RectanglesPadsCon, &m_ho_Rectangle5, m_hv_padc);
	SelectObj(m_ho_ContoursGrsmPc, &m_ho_CG, m_hv_padc);
	CountSeconds(&lht1);
	for (int i = 0; i < num; i++)
		ProcessMeander(m_ho_Im, m_ho_ContoursGrsmPc, m_ho_RectanglesPadsCon, m_ho_Gi,
